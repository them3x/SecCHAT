...
class cypher():
	def GetmasterKey(home_path, senha_usuario):
		with open(f"{home_path}.dchat/keys/keys.json", 'r') as file:
	...
	def GenAESkey(senha_usuario, home_path):
		...
		with open(f"{home_path}.dchat/keys/keys.json", 'w') as file:
	...
	def GenKey(self, home_path, aes_key, iv):
		...
		with open(f'{home_path}.dchat/keys/private_key.pem', 'wb') as f:
			f.write(self.encryptAES(private_key_pem, aes_key, iv))
			print ("[!] Chave publica criada")

		with open(f'{home_path}.dchat/keys/public_key.pem', 'wb') as f:
	...
	def GetKeys(self, home_path, aes_key, iv):
		with open(f'{home_path}.dchat/keys/private_key.pem', 'rb') as f:
			try:
				private_key_pem = self.decryptAES(f.read(), aes_key, iv).decode("UTF-8")
			except:
				private_key_pem = False

		with open(f'{home_path}.dchat/keys/public_key.pem', 'r') as f:
